import 'dart:async';
import 'dart:io';
import 'package:airpay_package_v3/model/Transaction.dart';
import 'package:airpay_package_v3/model/UserRequest.dart';
...
class _AirPayState extends State<AirPay> {
  ...
  String loadData() {
    ...
    privatekey = sha256.convert(temp);
    print(
        'privatekey ${widget.user.secret}@${widget.user.username}:|:${widget.user.password}');
    print('privatekey $privatekey');

    var setAllStr =
        '${widget.user.email}${widget.user.fname}${widget.user.lname}${widget.user.fulladdress}${widget.user.city}${widget.user.state}${widget.user.country}${widget.user.amount}${widget.user.orderid}$formattedDate';

    // key for checksum
    var sTemp2 =
        utf8.encode('${widget.user.username}~:~${widget.user.password}');
    var sKey = sha256.convert(sTemp2);
    // checksum
    var sAllData = utf8.encode('$sKey@$setAllStr');
    print('setAllData $sKey@$setAllStr');

    checksum = sha256.convert(sAllData);
  ...
  fetchDetails() async {
    ...
    var checksum2 = md5.convert(setAllData);
  ...
  userCancel(String withMessage) {
    ...
    trans.tRANSACTIONSTATUS = '500';
    trans.sTATUSMSG = withMessage;
